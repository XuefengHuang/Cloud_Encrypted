...
/// process 64 bytes
void SHA256_new::processBlock(const void* data)
{
  // get last hash
  uint32_t a = m_hash[0];
  uint32_t b = m_hash[1];
...
void SHA256_new::add(const void* data, size_t numBytes)
{
...
std::string SHA256_new::operator()(const void* data, size_t numBytes)
{
  reset();
  add(data, numBytes);
  return getHash();
...
std::string SHA256_new::operator()(const std::string& text)
{
  reset();
